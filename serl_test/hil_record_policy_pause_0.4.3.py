    ...
    def __call__(
        ...
        for name in motor_names:
            ...
            if key not in leader_action:
                raise KeyError(f"{key} is missing from leader_action.")
            if key not in self.follower_ref:
                raise KeyError(f"{key} is missing from follower_ref.")
...
def render_leader_follower_alignment(
    ...
    """Return a multi-line leader/follower alignment guide string."""
    leader_action = teleop.get_action()

    _, max_abs_diff = compute_leader_follower_diffs(
    ...
    lines.append(
        f"max_abs_diff(except gripper): {max_abs_diff:6.2f} deg   "
        f"threshold: {threshold_deg:6.2f} deg"
    )
    lines.append("Press 's' to start intervention.")
    lines.append("")
    ...
    if max_abs_diff <= threshold_deg:
        ...
        lines.append("")
        lines.append("Align leader closer to follower if possible. 's' will still start intervention.")
...
def clear_terminal_block(previous_num_lines: int) -> int:
    """Clear a previously rendered multi-line terminal block."""
    if previous_num_lines > 0:
        sys.stdout.write(f"\033[{previous_num_lines}F")
        sys.stdout.write("\033[J")
        sys.stdout.flush()
...
def record_loop(
    ...
    alignment_render_every_steps = 3
    ...
    print("HIL record_loop started")
    print("Controls:")
    print("  p: POLICY -> PAUSED_FOR_ALIGNMENT")
    print("  s: PAUSED_FOR_ALIGNMENT -> TELEOP_INTERVENTION")
    print("  e: TELEOP_INTERVENTION -> PAUSED_AFTER_INTERVENTION")
    print("  r: PAUSED_AFTER_INTERVENTION -> POLICY")
            ...
            if mode == ControlMode.POLICY:
                ...
                if key == "s":
                    if teleop is None:
                        ...
                        _, max_diff = compute_leader_follower_diffs(
                            leader_action=leader_action,
