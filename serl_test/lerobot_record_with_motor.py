...
class RecordConfig:
    ...
    resume: bool = False
    # Save follower motor status as sidecar CSV files.
    motor_log: bool = False
    motor_log_dir: str | None = None
    motor_log_every_frames: int = 1
...
def record_loop(
    ...
    while timestamp < control_time_s:
        ...
        if dataset is not None:
            ...
            # dataset.add_frame() の前に、現在のepisode内frame番号を取っておく
            episode_frame_index = motor_frame_idx

            # motor statusは、dataset frameと同じタイミングで保存する
            if (
                motor_log_file is not None
                and motor_log_every_frames > 0
                and episode_frame_index % motor_log_every_frames == 0
            ):
                ...
                motor_row = {
                    ...
                    "loop_time_s": float(time.perf_counter() - start_episode_t),
                }
                motor_row.update(flatten_motor_status("follower", follower_status))

                if motor_log_writer is None:
                    motor_log_writer = csv.DictWriter(
                        motor_log_file,
                        fieldnames=list(motor_row.keys()),
                    )
                    motor_log_writer.writeheader()

                motor_log_writer.writerow(motor_row)
        ...
        with VideoEncodingManager(dataset):
            ...
            while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
                ...
                try:
                    if cfg.motor_log:
                        #モーターの数値保存用ディレクトリの作成
                        if cfg.motor_log_dir is None:
                            motor_log_dir = Path(dataset.root) / "motor_logs"
                        else:
                            motor_log_dir = Path(cfg.motor_log_dir)

                        motor_log_dir.mkdir(parents=True, exist_ok=True)
                        motor_csv_path = motor_log_dir / f"episode_{episode_index:06d}_motor.csv"
                        motor_log_file = motor_csv_path.open("w", newline="")
                        logging.info(f"Motor log: {motor_csv_path}")
                    #記録
                    record_loop(
                        robot=robot,
                        events=events,
                        ...
                        motor_log_file=motor_log_file,
                        motor_log_every_frames=cfg.motor_log_every_frames,
                    )
                finally:
                ...
                if events["rerecord_episode"]:
                    ...
                    if motor_csv_path is not None and motor_csv_path.exists():
                        motor_csv_path.unlink()
                        logging.info(f"Deleted motor log for rerecord: {motor_csv_path}")

                    continue

                if events["stop_recording"]:
                    log_say("Stop recording: discard current episode", cfg.play_sounds)
                    dataset.clear_episode_buffer()

                    if motor_csv_path is not None and motor_csv_path.exists():
                        motor_csv_path.unlink()
                        logging.info(f"Deleted motor log for stopped episode: {motor_csv_path}")
